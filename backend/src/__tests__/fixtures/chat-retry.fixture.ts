...
let attempts=0
mock.module('../../services/agent/agentExecutor.js',()=>({chatWithAgent:async()=>{if(++attempts===1)throw new Error('503 Service Unavailable');return {reply:'接著討論',tokensUsed:1}}}))
...
test('a model failure permits a follow-up in the same active session',async()=>{
 ...
 await expect(chatInSession('s','retry',[],'owner')).resolves.toEqual({reply:'接著討論',tokensUsed:1})
})
