...
function parseInputSchema(schemaJson: string | null): InputField[] {
  ...
  try {
    ...
    return Object.entries(props).map(([name, def]: [string, any]) => ({
      ...
      label:
        def.title ||
        ({ query: '你的需求', _query: '你的需求' } as Record<string, string>)[name] ||
        name,
...
export default function AgentDetailPage() {
  ...
  useEffect(() => {
    ...
    fetchAgent(id)
      ...
      .catch(() => {
        setError('目前找不到這份服務，請回到列表選擇其他服務。')
        setLoading(false)
      })
  }, [id])
  ...
  const handleStartSession = async () => {
    ...
    try {
      if (agent.onchain_agent_id == null)
        throw new Error('這份服務尚未完成鏈上登記，目前無法開始使用。請選擇其他服務。')
      const deposit = Math.round(Number(budget) * 1000000)
      if (!Number.isFinite(deposit) || deposit < 100000 || deposit > 10000000)
        throw new Error('請設定 0.1 至 10 test USDC 的服務預算。')
  ...
  const handleRate = async (rating: number) => {
    ...
    try {
      ...
      setError(displayError(e, '評分尚未送出，請再試一次。'))
    }
  }

  if (loading)
    return (
      <div className="page-width page-section" role="status">
        正在載入服務介紹…
      </div>
    )
  if (!agent)
    return (
      <div className="page-width page-section">
        <div className="empty-state">
          <h1 className="text-2xl mb-4">{error || '目前找不到這份服務'}</h1>
      ...
      <div className="detail-layout">
        <div className="detail-intro">
          ...
          <div className="detail-explainer">
            ...
            <p>
              這份服務由提供者設定方法與內容，AI
              依照設定回應。說明你想處理的問題、已知條件，使用中也可以繼續補充與追問。
            </p>
          </div>
          <div className="detail-explainer">
            <h2>費用，你可以先設上限。</h2>
            <p>
              服務費率為{' '}
              <strong className="text-text-primary">{formatRate(agent.rate_per_second)}</strong>
              。開始後依使用計費，結束時由合約結算，未用完的預算退回錢包。請在離開前結束服務。
            </p>
          </div>
          <details className="technical-details">
            <summary>查看提供者與技術設定</summary>
          ...
          {address && (
            <div className="mt-6">
              {showRating ? (
                <div>
                  <span className="text-sm">你覺得這份服務如何？</span>
                  ...
                  {userRating && (
                    <p role="status" className="text-sm text-text-secondary">
                      已收到你的 {userRating} 分評價。
                    </p>
                  )}
                  <button className="text-link text-sm" onClick={() => setShowRating(false)}>
                    收起評分
                  </button>
                </div>
              ) : (
                <button className="text-link text-sm" onClick={() => setShowRating(true)}>
                  {userRating ? `你的評分：${userRating} / 5` : '留下你的評分'}
                </button>
              )}
            </div>
          )}
        ...
        >
          <h2>這次，想解決什麼？</h2>
          <p>說得具體一點，服務才能從你的情況出發。</p>
          {fields.length ? (
            fields.map((field) => (
              <div className="form-group" key={field.name}>
                <label htmlFor={`field-${field.name}`} className="field-label">
                  {field.label}
                  {field.required && <span className="text-accent text-xs ml-2">必填</span>}
                </label>
                <input
            ...
            <div className="form-group">
              <label htmlFor="field-query" className="field-label">
                你的需求<span className="text-accent text-xs ml-2">必填</span>
              </label>
              <textarea
                id="field-query"
                required
                className="field-input"
                value={inputs._query || ''}
                onChange={(e) => setInputs({ _query: e.target.value })}
                rows={5}
                placeholder="我想要…，目前的情況是…，最在意的是…"
              />
              <p className="field-hint">可以補充背景、限制，以及希望服務幫你釐清的問題。</p>
            </div>
          )}
          <div className="form-group border-t border-border-strong pt-6">
          ...
          {!address && (
            <div className="form-group">
              <p className="field-label">連接錢包，再確認開始</p>
              <ConnectWalletButton />
            </div>
          )}
          {error && (
          ...
          <button type="submit" className="button-primary" disabled={!address || starting}>
            {starting ? '正在確認錢包與建立服務…' : '確認預算，開始使用'}
            {!starting && <Icon />}
          </button>
          <p className="field-hint mt-4" role="status">
            {starting
              ? '請依錢包提示完成簽署與交易，完成後會進入對話。'
              : '目前使用 Base Sepolia 測試網。開始時需透過錢包授權並存入本次預算。'}
          </p>
        </form>
      </div>
    </div>
